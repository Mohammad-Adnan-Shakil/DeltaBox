...
| Feature | Description |
|---|---|
| 🧠 **Apex Intelligence** | AI race outcome prediction — blended XGBoost + Random Forest ensemble |
| 🔀 **Delta Analyst** | Lap-by-lap telemetry comparison between two drivers (speed, throttle, brake), sourced live from OpenF1 |
| 🏆 **Live Standings** | Driver and constructor standings, synced from Jolpica |
| 📅 **Race Calendar** | Full season with completed/upcoming status and team-colored podium results |
| 🛠️ **Race Engineer** | AI chat assistant for race strategy Q&A, powered by Groq (`llama-3.1-8b-instant`) |
...
↓ REST/HTTP
Flask ML Microservice (Render, separate deploy)
├── XGBoost model (.pkl)
...
**External APIs** — Jolpica (race/standings data), OpenF1 (telemetry), Groq (AI chat).

**Infra** — Render (backend + ML, Docker), Vercel (frontend), GitHub Actions (scheduled keep-alive to prevent free-tier idle spin-down).
